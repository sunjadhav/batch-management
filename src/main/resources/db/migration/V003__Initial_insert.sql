...
INSERT INTO `webflash_batch_management`.`user` VALUES(1,'admin', 'admin');

INSERT INTO `webflash_batch_management`.`batch` 
(`id`, `batch_id`, `name`, `timing`, `duration`, `started_at`, `is_active`)
 SELECT '1', 'B1', 'JAN20_8AM',
'8AM-10AM', '2 Months', '1579173855', 'Yes'  AS tmp
WHERE NOT EXISTS (
   SELECT `id` FROM `webflash_batch_management`.`batch` WHERE `id` = 1
) LIMIT 1;

INSERT INTO `webflash_batch_management`.`student` 
...
WHERE NOT EXISTS (
   SELECT id FROM `webflash_batch_management`.`student` WHERE id = '1'
) LIMIT 1;
